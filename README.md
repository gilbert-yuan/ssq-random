...
需要 Node.js 24 或更高版本。项目使用 Node.js 标准库和内置 `node:sqlite`，不需要安装第三方依赖。
...
```text
server.js                # 启动入口
src/server/              # 后端路由、SQLite、开奖源、指标、推荐补全
public/app.js            # 前端入口
public/js/domain/        # 前端分析和选号领域逻辑
public/js/components/    # 可复用 UI 组件和图表
public/js/pages/         # 页面编排
data/                    # 本地数据、样例、SQLite 运行库
```

## 本地记录

历史开奖、指标、推荐和命中核对数据保存在 `data/ssq.sqlite`。旧版 `data/records.json` 会在首次读取记录时迁移到 SQLite。数据库文件只在本机使用，已加入 `.gitignore`。
